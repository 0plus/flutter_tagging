...
class _FlutterTaggingState extends State<FlutterTagging> {
  ...
  ///init value to icon then mapping to json
  @override
  void initState() {
    super.initState();
    if (widget.isValue) {
            ...
            onDeleted: () {
              if (labeledChips.length > 0) {
                for (var chips in labeledChips) {
                  if (chips.key.toString().contains("${item.name}")) {
                    setState(() {
                      labeledChips.remove(chips);
                      _selectedTagValues.remove("${item.name}");
...
class TaggingData {
  String name;
  String value;
  TaggingData({this.name, this.value});
}
